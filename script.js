...
function loadTrackToPlayer(track) {
    ...
    if (track.type === 'blob' || track.type === 'arraybuffer') {
        const blob = track.type === 'arraybuffer' 
            ? new Blob([track.data], { type: track.mimeType || 'audio/mpeg' }) 
            : track.data;
...
function saveTrack(obj) {
    ...
    tx.onerror = e => {
        console.error("Erro ao salvar música no IndexedDB:", e.target.error);
        alert("Erro ao salvar música (limite de armazenamento excedido ou não suportado): " + e.target.error.message);
    };
}
...
function loadTrackToSeparator() {
    ...
    if (track.type === 'blob' || track.type === 'arraybuffer') {
        const blob = track.type === 'arraybuffer' 
            ? new Blob([track.data], { type: track.mimeType || 'audio/mpeg' }) 
            : track.data;
        currentSepObjectUrl = URL.createObjectURL(blob);
